...
const FeaturedRooms = () => {
  const { data } = api.transaction.getMostFeaturedRooms.useQuery();
  ...
  return (
    <Card className="flex-1 p-4">
      ...
      <CardContent className="grid md:grid-cols-3 gap-2 py-4">
        {data &&
          data.slice(0, 3).map((room, index) => (
            <Badge
              key={index}
              variant="outline"
              className="rounded-lg flex flex-col items-center gap-2 py-2"
            >
              <Image
