  ...
  function onSubmit(values: z.infer<typeof LostAndFoundSchema>) {
    if (userAvatar) {
      ...
      reader.onload = () => {
        ...
        handleMutation({ ...values, item_img: base64String });
      };
    } else {
      handleMutation({ ...values, item_img: undefined });
    }

    function handleMutation(formData: typeof values & { item_img?: string }) {
      toast.promise(
        lostAndFoundMutation.mutateAsync({
          ...formData,
          room_no: roomNo,
        }),
        {
          ...
          error: (error: unknown) => (error as Error).message,
        }
      );
    }
  }
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <span className="text-xs text-muted-foreground">
              Note: ensure the attached image is no larger than 2 MB.
            </span>
          </div>
        ...
        <SubmitButton mutation={lostAndFoundMutation}>
          Issue Lost Item
        </SubmitButton>
      </form>
    </Form>
